...
std::string ImGui::FileBrowser::GetLocal(const std::string& InPath)
{
	...
	return ImGuiStdStringRemove(relativePath.string(), "");
}
...
bool ImGui::FileBrowser::OpenInternal(const std::string&           InPath, FileBrowserOption InOption,
	...
	if (BeginPopupModal(GetLabel().c_str(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
	{
		...
		if (Button("Select", buttonSize))
		{
			...
			case FileBrowserOption::DIRECTORY:
				...
				break;
			case FileBrowserOption::FILE:
				OutSelectedPath = Selected.empty() ? OriginalPath : Path + "\\" + Selected;
				break;
			}
...
void ImGui::FileBrowser::EditNavigation()
{
	...
	{
		EditedPath == ".." ? TryPopPath() : TryApplyPath(GetEditedPath(EditedPath));
	}
	if (IsItemEdited())
		RefreshGuess();
	if (IsItemActive())
	{
		...
		{
			EditedPath == ".." ? TryPopPath() : TryApplyPath(NavigationGuess);
			// ClearActiveID();
		}
	}
